...
async function startServer() {
    ...
    app.post('/api/import', upload.single('file'), async (req, res) => {
        try {
            ...
            await (0, db_1.run)('INSERT INTO import_files (filename) VALUES ($1)', [req.file.originalname]);
            const importFileResult = await (0, db_1.get)('SELECT currval(pg_get_serial_sequence(\'import_files\', \'id\')) as id');
            const importFileId = importFileResult?.id || 0;
            for (const error of errors) {
                await (0, db_1.run)('INSERT INTO import_error_logs (import_file_id, index_number, english, reason) VALUES ($1, $2, $3, $4)', [importFileId, error.index, error.english, error.reason]);
            }
            await (0, db_1.run)('DELETE FROM word_relations');
            await (0, db_1.run)('DELETE FROM words');
            for (const word of words) {
                await (0, db_1.run)('INSERT INTO words (english, part_of_speech, chinese, is_classified) VALUES ($1, $2, $3, 0)', [word.english, word.part_of_speech, word.chinese]);
            }
            setTimeout(async () => {
                try {
                    ...
                    for (const word of allWords) {
                        ...
                        if (english.includes(' ')) {
                            ...
                            if (coreWord && coreWord !== word.id) {
                                const existing = await (0, db_1.get)('SELECT * FROM word_relations WHERE root_word_id = $1 AND child_word_id = $2 AND relation_type = $3', [coreWord, word.id, 'phrase']);
                                if (!existing) {
                                    await (0, db_1.run)('INSERT INTO word_relations (root_word_id, child_word_id, relation_type) VALUES ($1, $2, $3)', [coreWord, word.id, 'phrase']);
                                    wasClassified = true;
                                }
                            }
                        }
                            ...
                            if (rootWord && rootWord !== word.id) {
                                const existing = await (0, db_1.get)('SELECT * FROM word_relations WHERE root_word_id = $1 AND child_word_id = $2 AND relation_type = $3', [rootWord, word.id, 'derivative']);
                                if (!existing) {
                                    await (0, db_1.run)('INSERT INTO word_relations (root_word_id, child_word_id, relation_type) VALUES ($1, $2, $3)', [rootWord, word.id, 'derivative']);
                                    wasClassified = true;
                                }
                            }
                        }
                    ...
                    if (processedIds.length > 0) {
                        ...
                        await (0, db_1.run)(`UPDATE words SET is_classified = 1 WHERE id IN (${placeholders})`, processedIds);
                    }
                }
                catch (e) {
    ...
    app.get('/api/words', async (req, res) => {
        ...
        if (search) {
            ...
            total = await (0, db_1.get)('SELECT COUNT(*) as total FROM words WHERE english LIKE $1 OR chinese LIKE $2', [searchTerm, searchTerm]);
            total = total?.total || 0;
        }
        else {
            words = await (0, db_1.all)('SELECT * FROM words ORDER BY english LIMIT $1 OFFSET $2', [pageSize, offset]);
            total = await (0, db_1.get)('SELECT COUNT(*) as total FROM words');
            total = total?.total || 0;
        }
        ...
        const allChildWordIdsResult = await (0, db_1.all)('SELECT child_word_id FROM word_relations');
    ...
    app.get('/api/words/index/:wordId', async (req, res) => {
      ...
      `, [wordId]);
            const total = await (0, db_1.get)('SELECT COUNT(*) as total FROM words');
            res.json({
    ...
    app.delete('/api/words/:id', async (req, res) => {
        const wordId = parseInt(req.params.id);
        try {
            await (0, db_1.run)('DELETE FROM word_relations WHERE root_word_id = $1 OR child_word_id = $1', [wordId]);
            ...
            await (0, db_1.run)('DELETE FROM words WHERE id = $1', [wordId]);
            res.json({ success: true });
        }
        catch (error) {
    ...
    app.post('/api/words/batch-delete', async (req, res) => {
        ...
        try {
            ...
            await (0, db_1.run)(`DELETE FROM word_relations WHERE root_word_id IN (${placeholders}) OR child_word_id IN (${placeholders})`, [...wordIds, ...wordIds]);
            await (0, db_1.run)(`DELETE FROM error_words WHERE word_id IN (${placeholders})`, wordIds);
            await (0, db_1.run)(`DELETE FROM observation_words WHERE word_id IN (${placeholders})`, wordIds);
            await (0, db_1.run)(`DELETE FROM words WHERE id IN (${placeholders})`, wordIds);
    ...
    app.post('/api/practice/start', async (req, res) => {
        ...
        await (0, db_1.run)('INSERT INTO practice_sessions (status) VALUES ($1)', ['active']);
    ...
    app.post('/api/practice/end', async (req, res) => {
        ...
        if (sessionId) {
            await (0, db_1.run)('UPDATE practice_sessions SET status = $1, end_time = CURRENT_TIMESTAMP WHERE id = $2', ['completed', sessionId]);
        }
        else {
            await (0, db_1.run)('UPDATE practice_sessions SET status = $1, end_time = CURRENT_TIMESTAMP WHERE status = $2', ['completed', 'active']);
        }
        res.json({ success: true });
    });
    ...
    app.get('/api/words/batch-relations', async (req, res) => {
        ...
        const childIds = await (0, db_1.all)(`SELECT child_word_id FROM word_relations WHERE root_word_id IN (${placeholders})`, wordIds);
    ...
    app.post('/api/classify/reset', async (req, res) => {
        ...
        try {
            await (0, db_1.run)('DELETE FROM word_relations WHERE child_word_id = $1', [wordId]);
            await (0, db_1.run)('DELETE FROM word_relations WHERE root_word_id = $1', [wordId]);
            await (0, db_1.run)('UPDATE words SET is_classified = 0 WHERE id = $1', [wordId]);
            res.json({ success: true });
        }
        catch (error) {
    ...
    app.get('/api/db/status', async (req, res) => {
        const totalWordsResult = await (0, db_1.get)('SELECT COUNT(*) as count FROM words');
...
app.post('/api/parts-of-speech', async (req, res) => {
    ...
    try {
        await (0, db_1.run)('INSERT INTO parts_of_speech (code, name, description) VALUES ($1, $2, $3)', [code.trim(), name.trim(), description || '']);
...
app.put('/api/parts-of-speech/:id', async (req, res) => {
    ...
    try {
        await (0, db_1.run)('UPDATE parts_of_speech SET code = $1, name = $2, description = $3 WHERE id = $4', [code.trim(), name.trim(), description || '', id]);
...
app.post('/api/parts-of-speech/init-from-words', async (req, res) => {
    try {
        ...
        const existingCodes = existingCodesResult.map((p) => p.code.toLowerCase());
        const posFromWords = await (0, db_1.all)('SELECT DISTINCT part_of_speech FROM words WHERE part_of_speech IS NOT NULL AND part_of_speech != \'\'');
        const defaultPos = [
        ...
        for (const pos of defaultPos) {
            if (!existingCodes.includes(pos.code.toLowerCase())) {
                await (0, db_1.run)('INSERT INTO parts_of_speech (code, name, description) VALUES ($1, $2, $3)', [pos.code, pos.name, pos.description]);
                addedCount++;
        ...
        for (const item of posFromWords) {
            ...
            if (code && !existingCodes.includes(code.toLowerCase())) {
                await (0, db_1.run)('INSERT INTO parts_of_speech (code, name, description) VALUES ($1, $2, $3)', [code, code, '从导入数据中提取']);
                addedCount++;
